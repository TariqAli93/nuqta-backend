...
export class CompleteSetupWizardUseCase extends WriteUseCase<SetupWizardSettings, void, void> {
  ...
  async executeCommitPhase(input: SetupWizardSettings, _userId: string): Promise<void> {
    // modules, notifications and invoice are all optional — only write settings that
    // are explicitly provided. This allows calling with an empty body {} to simply
    // mark the wizard as started/completed without overwriting existing configuration.
    ...
    // Always mark wizard as completed
