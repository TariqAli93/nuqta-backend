...
// async function markerExists(productId: number, marker: string): Promise<boolean> {
//   const rows = await db
...
// function toBaseCost(unitCost: number, unitFactor: number, context: string): number {
//   if (unitFactor <= 1) {
//     return unitCost;
//   }
...
//   const existingProducts = await productRepo.findAll({ limit: 100_000, offset: 0 });
...
//     if (existing) {
//       return { entity: requireId(existing, `user:${input.username}`), created: false };
//     }
...
//     if (existing) {
//       return { entity: requireId(existing, `product:${input.sku}`), created: false };
//     }
...
//       const allBatches = await productRepo.findBatchesByProductId(item.productId);
...
//       if ((productWithId.status ?? "available") === "out_of_stock" && stockAfter > 0) {
...
//       await upsertSetting("barcode.defaultTemplateId", String(defaultTemplate.id));
//     }
//   }
...
//   const repairedSupplierBalance = await reconcileSupplierBalanceUseCase.repair();
