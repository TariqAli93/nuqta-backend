...
export const accountingRoundingMethodEnum = pgEnum(
  "accounting_rounding_method",
  ["round", "floor", "ceil"],
);
...
export const suppliers = pgTable(
  ...
  (table) => [
    check(
      "chk_suppliers_opening_balance_nonnegative",
...
export const products = pgTable(
  ...
  (table) => [
    ...
    index("idx_products_low_stock").on(
      table.isActive,
      table.stock,
      table.minStock,
    ),
    ...
    check(
      "chk_products_selling_price_nonnegative",
...
export const productBatches = pgTable(
  ...
  (table) => [
    ...
    index("idx_product_batches_product_expiry").on(
      table.productId,
      table.expiryDate,
    ),
    index("idx_product_batches_product_quantity").on(
      table.productId,
      table.quantityOnHand,
    ),
    uniqueIndex("idx_batches_unique").on(table.productId, table.batchNumber),
    check(
      "chk_batches_qty_received_nonnegative",
      sql`${table.quantityReceived} >= 0`,
    ),
    check(
      "chk_batches_qty_on_hand_nonnegative",
...
export const sales = pgTable(
  ...
  (table) => [
    ...
    check(
      "chk_sales_remaining_nonnegative",
      sql`${table.remainingAmount} >= 0`,
    ),
    check("chk_sales_exchange_rate_positive", sql`${table.exchangeRate} > 0`),
    check(
      "chk_sales_interest_rate_nonnegative",
      sql`${table.interestRate} >= 0`,
    ),
    check(
      "chk_sales_interest_amount_nonnegative",
      sql`${table.interestAmount} >= 0`,
    ),
  ],
);
...
export const saleItems = pgTable(
  "sale_items",
  {
    ...
    discount: integer("discount").notNull().default(0),
    subtotal: integer("subtotal").notNull(),
    returnedQuantityBase: integer("returned_quantity_base")
      .notNull()
      .default(0),
    createdAt: timestamp("created_at", { mode: "string" }).defaultNow(),
  },
  (table) => [
    ...
    check(
      "chk_sale_items_unit_price_nonnegative",
...
export const saleItemDepletions = pgTable(
  ...
  (table) => [
    ...
    check(
      "chk_sale_item_depletions_quantity_positive",
      sql`${table.quantityBase} > 0`,
    ),
    check(
      "chk_sale_item_depletions_cost_nonnegative",
      sql`${table.costPerUnit} >= 0`,
    ),
    check(
      "chk_sale_item_depletions_total_nonnegative",
...
export const purchases = pgTable(
  ...
  (table) => [
    ...
    check(
      "chk_purchases_remaining_nonnegative",
      sql`${table.remainingAmount} >= 0`,
    ),
    check(
      "chk_purchases_exchange_rate_positive",
...
export const purchaseItems = pgTable(
  ...
  (table) => [
    ...
    check(
      "chk_purchase_items_unit_factor_positive",
      sql`${table.unitFactor} >= 1`,
    ),
    check(
      "chk_purchase_items_quantity_base_positive",
      sql`${table.quantityBase} > 0`,
    ),
    check(
      "chk_purchase_items_unit_cost_nonnegative",
      sql`${table.unitCost} >= 0`,
    ),
    check(
      "chk_purchase_items_discount_nonnegative",
      sql`${table.discount} >= 0`,
    ),
    check(
      "chk_purchase_items_line_subtotal_nonnegative",
      sql`${table.lineSubtotal} >= 0`,
    ),
  ],
);
...
export const payments = pgTable(
  ...
  (table) => [
    ...
    check(
      "chk_payments_exchange_rate_positive",
      sql`${table.exchangeRate} > 0`,
    ),
...
export const inventoryMovements = pgTable(
  ...
  (table) => [
    ...
    index("idx_inventory_movements_product_created_at").on(
      table.productId,
      table.createdAt,
    ),
    index("idx_inv_mov_source").on(table.sourceType, table.sourceId),
    check(
      "chk_inventory_movements_quantity_nonzero",
      sql`${table.quantityBase} <> 0`,
    ),
    check(
      "chk_inventory_movements_unit_factor_positive",
      sql`${table.unitFactor} >= 1`,
    ),
    check(
    ...
    check(
      "chk_inventory_movements_stock_before_nonnegative",
      sql`${table.stockBefore} >= 0`,
    ),
    check(
      "chk_inventory_movements_stock_after_nonnegative",
...
export const payrollRuns = pgTable(
  ...
  (table) => [
    ...
    check(
      "chk_payroll_runs_period_year",
      sql`${table.periodYear} BETWEEN 2000 AND 9999`,
    ),
    check(
      "chk_payroll_runs_period_month",
      sql`${table.periodMonth} BETWEEN 1 AND 12`,
    ),
    check(
      "chk_payroll_runs_gross_nonnegative",
      sql`${table.totalGrossPay} >= 0`,
    ),
    check(
      "chk_payroll_runs_deductions_nonnegative",
      sql`${table.totalDeductions} >= 0`,
    ),
    check(
      "chk_payroll_runs_bonuses_nonnegative",
      sql`${table.totalBonuses} >= 0`,
    ),
...
export const postingBatches = pgTable(
  ...
  {
    id: serial("id").primaryKey(),
    periodType: postingBatchPeriodTypeEnum("period_type")
      .notNull()
      .default("day"),
  ...
  (table) => [
    ...
    check(
      "chk_posting_batches_entries_nonnegative",
      sql`${table.entriesCount} >= 0`,
    ),
    check(
      "chk_posting_batches_total_nonnegative",
      sql`${table.totalAmount} >= 0`,
    ),
    check(
      "chk_posting_batches_period_order",
...
export const journalEntries = pgTable(
  "journal_entries",
  {
    ...
    postingBatchId: integer("posting_batch_id").references(
      () => postingBatches.id,
      {
        onDelete: "set null",
        onUpdate: "cascade",
  ...
  (table) => [
    ...
    check(
      "chk_journal_entries_total_nonnegative",
      sql`${table.totalAmount} >= 0`,
    ),
  ],
);
...
export const payrollRunItems = pgTable(
  ...
  (table) => [
    ...
    check(
      "chk_payroll_run_items_gross_nonnegative",
      sql`${table.grossPay} >= 0`,
    ),
    check(
      "chk_payroll_run_items_deductions_nonnegative",
      sql`${table.deductions} >= 0`,
    ),
    check(
      "chk_payroll_run_items_bonuses_nonnegative",
      sql`${table.bonuses} >= 0`,
    ),
...
export const customerLedger = pgTable(
  ...
  (table) => [
    ...
    index("idx_customer_ledger_customer_created_at").on(
      table.customerId,
      table.createdAt,
    ),
...
export const supplierLedger = pgTable(
  ...
  (table) => [
    ...
    index("idx_supplier_ledger_supplier_created_at").on(
      table.supplierId,
      table.createdAt,
    ),
...
export const barcodeTemplates = pgTable(
  "barcode_templates",
  {
    id: serial("id").primaryKey(),
    name: text("name").notNull(),
...
export const barcodePrintJobs = pgTable(
  ...
  (table) => [
    ...
    check(
      "chk_barcode_print_jobs_quantity_positive",
...
export const currencySettings = pgTable(
  "currency_settings",
  {
    id: serial("id").primaryKey(),
  ...
  (table) => [
    check(
      "chk_currency_settings_exchange_rate_positive",
      sql`${table.exchangeRate} > 0`,
    ),
  ],
);
...
export const systemSettings = pgTable(
  ...
  (table) => [
    check(
      "chk_system_settings_low_stock_threshold_nonnegative",
      sql`${table.lowStockThreshold} >= 0`,
    ),
    check(
      "chk_system_settings_expiry_alert_days_nonnegative",
      sql`${table.expiryAlertDays} >= 0`,
    ),
    check(
      "chk_system_settings_debt_reminder_count_nonnegative",
      sql`${table.debtReminderCount} >= 0`,
    ),
    check(
...
export const accountingSettings = pgTable(
  "accounting_settings",
  {
    ...
    costMethod: accountingCostMethodEnum("cost_method")
      .notNull()
      .default("fifo"),
  ...
  (table) => [
    check(
      "chk_accounting_settings_default_tax_rate_nonnegative",
      sql`${table.defaultTaxRate} >= 0`,
    ),
    check(
      "chk_accounting_settings_usd_rate_positive",
      sql`${table.usdExchangeRate} > 0`,
    ),
    check(
...
export const barcodeSettings = pgTable(
  ...
  (table) => [
    check(
      "chk_barcode_settings_default_width_positive",
      sql`${table.defaultWidth} > 0`,
    ),
    check(
      "chk_barcode_settings_default_height_positive",
      sql`${table.defaultHeight} > 0`,
    ),
    check(
      "chk_barcode_settings_print_dpi_positive",
      sql`${table.printDpi} > 0`,
    ),
    check(
      "chk_barcode_settings_label_width_positive",
      sql`${table.labelWidthMm} > 0`,
    ),
    check(
      "chk_barcode_settings_label_height_positive",
      sql`${table.labelHeightMm} > 0`,
    ),
    check(
      "chk_barcode_settings_margin_top_nonnegative",
      sql`${table.marginTop} >= 0`,
    ),
    check(
      "chk_barcode_settings_margin_bottom_nonnegative",
      sql`${table.marginBottom} >= 0`,
    ),
    check(
      "chk_barcode_settings_margin_left_nonnegative",
      sql`${table.marginLeft} >= 0`,
    ),
    check(
      "chk_barcode_settings_margin_right_nonnegative",
      sql`${table.marginRight} >= 0`,
    ),
  ],
);
